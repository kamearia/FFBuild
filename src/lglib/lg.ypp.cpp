...
list_of_id_args: /* returns [[YYSTYPE_clist_id]] */
  ...
  | FESPACEL id                                { $$ = new ListOfId(); $$->push_back(UnId($2,Find($1),atype<FE<double,5> **>()));}
  | FESPACEL '&' id                            { $$ = new ListOfId(); $$->push_back(UnId($3,Find($1),atype<FE<double,5> **>(),true));}
  ...
  | list_of_id_args ',' FESPACEL id            { $$ = $1; $$->push_back(UnId($4,Find($3),atype<FE<double,5> **>()));}
  | list_of_id_args ',' FESPACEL '&' id        { $$ = $1; $$->push_back(UnId($5,Find($3),atype<FE<double,5> **>(),true));}
...
id: ID | FESPACE|FESPACE3|FESPACES|FESPACEL|FESPACE1;
...
primary: /* returns a [[YYSTYPE_cexp]] */
  ...
  |        TYPE '('  parameterstype ')' {
           { $$=$1->right()->Find("<--",basicAC_F0_wa($3));
           if (!$$.left()) { cerr << " no wait to change (args) in " <<
                                      $1->right()->name() << endl;
                              CompileError(" Error in type(exp) "); }
           }
          }
