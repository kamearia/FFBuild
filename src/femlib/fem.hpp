...
namespace Fem2D {
  ...
  // definition of triangle numerotation, trigonometric direction
  // the edge is opposite of the vertex
  // [3] is an edge
...
/*
  class Tetraedre: public Label {
    ...
    public:
      ...
      R3 N2areaInternal(int i) const {
        return R3(*vertices[v_tet_face[i][0]], *vertices[v_tet_face[i][1]])
        ^R3(*vertices[v_tet_face[i][0]], *vertices[v_tet_face[i][2]]);
      }

      R3 n(int i) const { // unit exterior normal
        R3 Ni = N2areaInternal(i);
        return Ni/-Norme2(Ni);
      }

      R3 H(int i) const { // heigth (nabla lambda_i
        R3 Ni = N2areaInternal(i);
        return Ni/(3.*volume);
      }
      ...
      R h() const {
        return Max(Max(lenEdge(0), lenEdge(1), lenEdge(2)), Max(lenEdge(3), lenEdge(4), lenEdge(5)));
      }
...
*/
  ...
  class TTriangle: public Label {
    ...
    public:
      ...
      Vertex & Edge(int j, int i) const { // Vertex j of edge i
        throwassert(j == 0 || j == 1 );
        return *vertices[(i+j+1)%3];
      }

      // il y a un problem sur d=3 ici ----
      Rd n(int i) const { // unit exterior normal
        Rd E = Edge(i);
        return Rd(E.y, -E.x)/Norme2(E);
      }

      Rd H(int i) const { // heigth nabla lambda_i
        Rd E = Edge(i);
        return Rd(-E.y, E.x)/(2*area);
      }
    ...
    public:
     ...
     // Tetraedre *tet; //
      ...
      int ElementAdj(int it, int &j) const {
        int i = TheAdjacencesLink[3*it+j];
        j = i%3;
        return i/3;
      }
      ...
      int BoundaryElement(int bbe, int &edgeInT) const {
        int i = BoundaryEdgeHeadLink[bbe];
        edgeInT = i%3;
        return i/3;
      }
  ...
  void HeapSort(T *c, long n) {
    long l,j,r,i;
    T crit;
    c--; // we shift from 1 to start the table at index 1
    if (n <= 1) return;
    l = n/2 + 1;
    r = n;
    while (1) { // label 2
  ...
  void HeapSort(T *c, T1 *c1, T2 *c2, long n) {
    ...
    c--; c1--; c2--; // we shift from 1 to start the table at index 1
    if( n <= 1) return;
    l = n/2 + 1;
    r = n;
    while (1) { // label 2
  ...
  inline R2 minmax(const R2 &a, const R2 &b) { return R2(Min(a.x, b.x), Max(a.y, b.y)); }
}
